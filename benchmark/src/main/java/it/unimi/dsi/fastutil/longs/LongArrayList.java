/*
 * Copyright (c) 2002-2021. The Meowool Organization Open Source Project
 *
 ...
 * (un)boxing.
 *
 * <p>This class implements a lightweight, fast, open, optimized, reuse-oriented version of
 * array-based lists. Instances of this class represent a list with an array that is enlarged as
 * needed when new entries are created (by increasing its current length by 50%), but is
 * <em>never</em> made smaller (even on a {@link #clear()}). A family of {@linkplain #trim()
 * trimming methods} lets you control the size of the backing array; this is particularly useful if
 * you reuse instances of this class. Range checks are equivalent to those of {@code java.util}'s
 * classes, but they are delayed as much as possible. The backing array is exposed by the {@link
 * #elements()} method.
 *
 * <p>This class implements the bulk methods {@code removeElements()}, {@code addElements()} and
 * {@code getElements()} using high-performance system calls (e.g., {@link
 * System#arraycopy(Object,int,Object,int,int) System.arraycopy()}) instead of expensive loops.
 *
...
public class LongArrayList extends AbstractLongList
    ...
    /**
     * Creates a new array list and fills it with the elements returned by a type-specific
     * iterator..
     *
     ...
     *
     * <p>Note it is guaranteed that the type of the array returned by {@link #elements()} will be
     * the same (see the comments in the class documentation).
     *
     ...
     *
     * @apiNote Taking a primitive stream instead of returning something like a {@link
     *     java.util.stream.Collector Collector} is necessary because there is no primitive {@code
     *     Collector} equivalent in the Java API.
     */
    ...
    /**
     * Collects the result of a primitive {@code Stream} into a new ArrayList, potentially
     * pre-allocated to handle the given size.
     *
     * <p>This method performs a terminal operation on the given {@code Stream}
     *
     * @apiNote Taking a primitive stream instead returning something like a {@link
     *     java.util.stream.Collector Collector} is necessary because there is no primitive {@code
     *     Collector} equivalent in the Java API.
     */
    public static LongArrayList toListWithExpectedSize(
            java.util.stream.LongStream stream, int expectedSize) {
        if (expectedSize <= DEFAULT_INITIAL_CAPACITY) {
            // Already below default capacity. Just use all default construction instead of fiddling
            // with atomics in SizeDecreasingSupplier
            return toList(stream);
        }
        return stream.collect(
                new LongCollections.SizeDecreasingSupplier<LongArrayList>(
                        expectedSize,
                        (int size) ->
                                size <= DEFAULT_INITIAL_CAPACITY
                                        ? new LongArrayList()
                                        : new LongArrayList(size)),
                LongArrayList::add,
    ...
    public void ensureCapacity(final int capacity) {
        if (capacity <= a.length
                || (a == LongArrays.DEFAULT_EMPTY_ARRAY && capacity <= DEFAULT_INITIAL_CAPACITY))
            return;
     ...
     * resizing, and in case increasing the current capacity at least by a factor of 50%.
     *
     * @param capacity the new minimum capacity for this array list.
     */
        ...
        if (a != LongArrays.DEFAULT_EMPTY_ARRAY)
            capacity =
                    (int)
                            Math.max(
                                    Math.min(
                                            (long) a.length + (a.length >> 1),
                                            it.unimi.dsi.fastutil.Arrays.MAX_ARRAY_SIZE),
                                    capacity);
     ...
     *
     * <p>If the current array length is smaller than or equal to {@code n}, this method does
     * nothing. Otherwise, it trims the array length to the maximum between {@code n} and {@link
     * #size()}.
     *
     * <p>This method is useful when reusing lists. {@linkplain #clear() Clearing a list} leaves the
    ...
    private class SubList extends AbstractLongList.LongRandomAccessSubList {
        ...
        // Most of the inherited methods should be fine, but we can override a few of them for
        // performance.
        // Needed because we can't access the parent class' instance variables directly in a
        // different instance of SubList.
        ...
        private final class SubListIterator extends LongIterators.AbstractIndexBasedListIterator {
            // We are using pos == 0 to be 0 relative to SubList.from (meaning you need to do a[from
            // + i] when accessing array).
        ...
        boolean contentsEquals(long[] otherA, int otherAFrom, int otherATo) {
            ...
            // We have already assured that the two ranges are the same size, so we only need to
            // check one bound.
            // TODO When minimum version of Java becomes Java 9, use the Arrays.equals which takes
            // bounds, which is vectorized.
        ...
        int contentsCompareTo(long[] otherA, int otherAFrom, int otherATo) {
            ...
            // TODO When minimum version of Java becomes Java 9, use Arrays.compare, which
            // vectorizes.
            long e1, e2;
            int r, i, j;
     ...
     * {@inheritDoc}
     *
     * <p>The returned spliterator is late-binding; it will track structural changes after the
     * current index, up until the first {@link java.util.Spliterator#trySplit() trySplit()}, at
     * which point the maximum index will be fixed. <br>
     * Structural changes before the current index or after the first {@link
     * java.util.Spliterator#trySplit() trySplit()} will result in unspecified behavior.
     */
    @Override
    public LongSpliterator spliterator() {
    ...
    public LongArrayList clone() {
        ...
        if (getClass() == LongArrayList.class) {
            // Preserve backwards compatibility and make new list have Object[] even if it was
            // wrapped from some subclass.
     ...
     *
     * @apiNote This method exists only for sake of efficiency. The implementation inherited from
     *     the abstract implementation would already work.
     ...
     * @return a negative integer, zero, or a positive integer as this list is lexicographically
