 ...
 *
 * [Abstract reference java new io](https://github.com/openjdk/jdk/blob/master/src/java.base/share/classes/java/nio/file/Path.java)
 * [Abstract reference multiplatform okio](https://github.com/square/okio/blob/master/okio/src/commonMain/kotlin/okio/Path.kt)
 * [Abstract reference python os.path](https://github.com/python/cpython/blob/3.9/Lib/posixpath.py)
 *
 * @param Actual represents the actual return type of members (such as file, directory, etc...).
 * @author 凛 (https://github.com/RinOrz)
 */
interface BasePath<Actual: BasePath<Actual>> : Comparable<Actual> {
   ...
   */
  val symbolicLink: Actual

  /**
   * The name of the file or directory of this path, which is the last element of [BasePath.split].
   */
  var name: String
  ...
  /**
   * The instant of last modification.
   *
   * If the file system implementation does not support a time stamp to indicate the time of
   * last modification then this property returns an implementation specific default value,
   * typically milliseconds representing the epoch (1970-01-01T00:00:00Z).
   */
  var lastModifiedInstant: Instant
  ...
  /**
   * The instant of last access.
   *
   * If the file system implementation does not support a time stamp to indicate the time of
   * last access then this property returns an implementation specific default value, typically
   * the [lastModifiedInstant] or milliseconds representing the epoch (1970-01-01T00:00:00Z).
   */
  var lastAccessInstant: Instant
  ...
  /**
   * The creation instant is the time that the file was created.
   *
   * If the file system implementation does not support a time stamp to indicate the time when
   * the file was created then this property returns an implementation specific default value,
   * typically the [lastAccessInstant] or milliseconds representing the epoch (1970-01-01T00:00:00Z).
   */
  ...
  /**
   * Returns an object that uniquely identifies the given path, or `null` if a key is not available.
   * On some platforms or file systems it is possible to use an identifier, or a combination of
   * identifiers to uniquely identify a path. Such identifiers are important for operations such as
   * file tree traversal in file systems that support symbolic links  or file systems that allow a
   * file to be an entry in more than one directory. On UNIX file systems, for example, the
   * 'device ID' and 'inode' are commonly used for such purposes.
   *
   * The key returned by this method can only be guaranteed to be unique if the file system and
   * files remain static. Whether a file system re-uses identifiers after a file is deleted is
   ...
   * @param target the target path to be reached by this path.
   */
  infix fun relativeTo(target: Path): Path = relativeTo(target.toString())
  ...
  fun createParentDirectories(): Actual
   ...
   * @return this path
   */
  fun toReal(followLinks: Boolean = true): Actual
   ...
   */
  override fun compareTo(other: Actual): Int
 ...
 * @param more additional char sequence to be joined to form the path
 */
@JvmName("nullableAsPath")
inline fun CharSequence?.asPath(vararg more: CharSequence): Path? = this?.asPath(*more)
